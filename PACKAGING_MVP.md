...
- Packaging wrappers also verify that generated distribution archives and installers are real non-empty files before reporting success or signing them. Windows zip verification checks the packaged executable for PyInstaller builds and `source-package\app_update_config.json` for source-package fallback builds. Windows signing fails if a requested package folder contains no signable `.exe`, `.dll`, or `.pyd` artifacts.
